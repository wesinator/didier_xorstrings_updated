...
void DoStringsAnalysis(void *pBuffer, long lSize, const char *pcArgFile, int iFlagSave, int iFlagDump, int iFlagMaxSort, int iMinimumStringLength, int iTerminator, int iFlagCSV)
{
    ...
    puts(iFlagCSV ? "Operation,Key,Number of strings,Average string length,Max string length" : "Operation   Key  Number of strings Average string length   Max string length");
...
main(int argc, char **argv)
{
    ...
    "XORStrings-ng V0.0.2, look for XOR, ROL, or SHIFT encoded strings in a file\n\n"
    "Use -s to save every rotation of XOR, ROL, or SHIFT encoding to a file\n"
